...
func RollbackTenderVersionHandler(w http.ResponseWriter, r *http.Request) {
	...
	tenderResponse, statusCode, err := dbStruct.RollbackTenderVersion()
	if err != nil {
		manager.Mng.Log.LogError("Ошибка при смене версии тендера в БД", err)
		handlers.SendErrorResponse(w, fmt.Sprintf("Ошибка при смене версии тендера в БД: %s", err), statusCode)
		return
	}
